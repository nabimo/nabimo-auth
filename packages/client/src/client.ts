...
import { AuthClientError, type AuthClientOptions, type AuthRequestOptions, type AuthenticationResult, type AuthTokens, type LogoutAllResult, type LogoutResult, type PasswordResetConfirmResult, type PasswordResetRequestResult, type TokenStorage, type TwoFactorResult, type TwoFactorSetupResult, type VerificationChallengeResult, type VerificationResult } from "./types.js";
import type { AuthErrorResponse, PasswordLoginRequest, PasswordResetConfirmRequest, PasswordResetRequest, PhoneVerificationRequest, RefreshRequest, RegisterRequest, TwoFactorCodeRequest } from "@nabimo-auth/protocol";
...
export class AuthClient {
  ...
  private async authenticate(path: string, body: RegisterRequest | PasswordLoginRequest): Promise<AuthenticationResult> { const result = await this.post<AuthenticationResult>(path, body); await this.storage.set({ accessToken: result.accessToken, refreshToken: result.refreshToken }); return result; }
